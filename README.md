...
```
题目 ──→ [大纲子智能体] ──→ 可用提纲 ──→ [写作 Agent] ──→ 草稿 ──→ [审校/润色 Agent] ──→ 成品文章
             │（自包含）                        ↑____________（不合格则打回重写，最多 2 次）____________|
             └─ 搜索 → 审查素材 → 生成提纲 → 自检 →（素材不足则补搜 / 提纲不合格则重试）
```

- **大纲子智能体**（自包含独立子图）：一次到位——① 让模型一次提出 2-4 个**具体聚焦**的查询，调用 `web_search`（DuckDuckGo，`region=cn-zh`）联网搜索；② 用 `REVIEWER_PROMPT` 让 LLM **审查搜索结果**，剔除营销味/宽泛/脏数据，整理出可靠素材；③ 基于素材生成提纲并自检（非空 + 够长）。**素材不足会自动补搜（≤2 轮），提纲不合格会换提示重试（≤2 次），仍不行用自身知识兜底**——搜索、审查、提纲三者闭环，保证返回的提纲一定可用。
...
├── graph.py                # LangGraph 编排：大纲 → 写作 → 审校（含打回循环）
└── main.py                 # CLI 入口
```
...
```
  🔍 大纲子智能体搜索素材（第 1 轮）…
    🔍 搜索：多智能体系统 定义 核心概念 自主性 协作
    🔍 搜索：多智能体 应用案例 自动驾驶 机器人 交通调度
  🧐 审查搜索结果素材…
  🔍 大纲子智能体搜索素材（第 2 轮）…        ← 第 1 轮素材不足，自动补搜
    🔍 搜索：多智能体系统 定义 基本概念 智能体
    🔍 搜索：多智能体 大语言模型 LLM Agent 协作 框架
  🧐 审查搜索结果素材…
  📋 大纲子智能体生成提纲（第 1 次）…
→ 写作中…
→ 审校/润色中…
...
3. **大纲子智能体（`agents/outliner.py`）**：一个编译好的**自包含子图**，挂到主图的 `outline` 节点，负责"检索 + 生成提纲"一体：
   - **搜索**：把对话交给模型（带 `web_search` 工具），模型一次并行提出 2-4 个具体查询，逐个执行搜索；
